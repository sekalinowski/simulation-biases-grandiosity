...
S1_data_input <- "NaM_Study1_Github_Memory.csv"
Rep_data_input <- "NaM_Rep_Github_Memory.csv"
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Plot Study 1 Subjective Detail Models ####
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# In this section, we create Fig 1 and the supplementary figure from the Study 1 subjective detail models

library(ggpubr)

# Extract model estimates
S1_difficultyEstimates <- as.data.frame(effects::Effect(c("FFNI_Grandiose","valence","taskName"), S1_difficultmodel))
S1_vividnessEstimates <- as.data.frame(effects::Effect(c("FFNI_Grandiose","valence","taskName"), S1_vividmodel))
S1_freqEstimates <- as.data.frame(effects::Effect(c("FFNI_Grandiose","valence"), S1_freqmodel))
S1_plausEstimates <- as.data.frame(effects::Effect(c("FFNI_Grandiose","valence"), S1_plausmodel))
S1_perspEstimates <- as.data.frame(effects::Effect(c("FFNI_Grandiose","valence","taskName"), S1_perspectmodel))


# Difficulty plot
diffPlot <- ggplot(S1_difficultyEstimates, aes(x = FFNI_Grandiose, y = eventDifficulty, color = valence, linetype = taskName)) +
  geom_ribbon(aes(FFNI_Grandiose, fit, ymin = lower, ymax = upper), fill = "grey70", alpha = 0.2, size = 0) +
  geom_line(aes(FFNI_Grandiose, fit), size = 1.5) +
  theme_classic() + scale_linetype_manual(values = c("dotted","solid"), labels = c("FUTURE","MEMORY"))+
  xlab("Grandiosity") +
  ylab("Difficulty") + 
  labs(linetype = 'Task') +
  labs(color = 'Valence') +
  ...
  scale_color_manual(values=c("#FF3333", "#0072B2"),labels = c("NEGATIVE","POSITIVE"))

# Vividness plot
vivPlot <- ggplot(S1_vividnessEstimates, aes(x = FFNI_Grandiose, y = eventVividness, color = valence, linetype = taskName)) +
  geom_ribbon(aes(FFNI_Grandiose, fit, ymin = lower, ymax = upper),fill = "grey70", alpha = 0.2, size = 0) +
  geom_line(aes(FFNI_Grandiose, fit), size = 1.5) +
  theme_classic() + scale_linetype_manual(values = c("dotted","solid"), labels = c("FUTURE","MEMORY"))+
  xlab("Grandiosity") +
...
# Frequency plot
freqPlot <- ggplot(S1_freqEstimates, aes(x = FFNI_Grandiose, y = eventThoughtAboutBefore, color = valence)) +
  geom_ribbon(aes(FFNI_Grandiose, fit, ymin = lower, ymax = upper), fill = "grey70", alpha = 0.2, size = 0) +
  geom_line(aes(FFNI_Grandiose, fit), size = 1.5, linetype = "dotted") +
  theme_classic() +
  xlab("Grandiosity") +
  ylab("Frequency") + 
  labs(color = 'Valence') +
...
# Plausibility plot
plausPlot <- ggplot(S1_plausEstimates, aes(x = FFNI_Grandiose, y = eventPlausibility, color = valence)) +
  geom_ribbon(aes(FFNI_Grandiose, fit, ymin = lower, ymax = upper), fill = "grey70", alpha = 0.2, size = 0) +
  geom_line(aes(FFNI_Grandiose, fit), size = 1.5, linetype = "dotted") +
  theme_classic() +
  xlab("Grandiosity") +
...
figure <- ggarrange(diffPlot, vivPlot, freqPlot, plausPlot,
                    labels = c("A","B","C","D"),
                    ncol = 2, nrow = 2,
                    common.legend = TRUE,
                    legend = "right")
figure
ggsave(plot = figure, filename = "figure1.png", device = "png")

# Make supplementary fig for visual perspective
perspecPlot <- ggplot(S1_perspEstimates, aes(x = FFNI_Grandiose, y = eventVisPerspective, color = valence, linetype = taskName)) +
  geom_ribbon(aes(FFNI_Grandiose, fit, ymin = lower, ymax = upper),fill = "grey70", alpha = 0.2, size = 0) +
  geom_line(aes(FFNI_Grandiose, fit), size = 1.5) +
  theme_classic() + scale_linetype_manual(values = c("dotted","solid"), labels = c("FUTURE","MEMORY"))+
  xlab("Grandiosity") +
  ylab("Visual Perspective") + 
  labs(linetype = 'Task') +
  labs(color = 'Valence') +
  ylim(1,7) +
  theme(legend.key.width = unit(1, 'cm')) +
  scale_color_manual(values=c("#FF3333", "#0072B2"), labels = c("NEGATIVE","POSITIVE")) 
perspecPlot
ggsave(plot = perspecPlot, filename = "suppFig1.png", device = "png")

#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Objective Detail Analyses ####
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# We first compute the agreement between the automated and manual scoring approaches using correlations and
#     intraclass correlation. We then estimate a series of linear mixed effects models, predicting the internal 
#     and external detail from written narratives describing remembered past and imagined future events. 
#     Internal detail represents the amount of episodic detail provided in the event, and external detail 
#     reflects non-episodic detail according to the Autobiographical Interview (Levine et al., 2002). 
#     Internal and external detail metrics were acquired with the automated Autobiographical Interview 
#     scoring tool (van Genugten & Schacter, in press).

# Get the subset of 60 events in each dataset that were scored manually
...
### We first assess reliability of manual and automated scoring methods using correlations, as were reported in
...
##### Cross-correlations
### Study 1
cor.test(S1_scoringDF$manualInternalDetails, S1_scoringDF$numExternalDetails) # -0.072, n.s.
...
### Assess reliability of manual and automated scoring using intraclass correlations
# This requires that we scale the data so that automated and manual scores are on the same scale, then
# put those scaled values into an icc. We do this in the section below for Study 1 internal and external details
# and Replication internal and external details.

# Here we create dataframes for each detail type for each study. numInternalDetails/numExternalDetails refers 
#   to output from the automated scoring, manualInternalDetails and manualExternal details refer to the manual 
#   AI scores
S1_intScoreDF <- subset(S1_scoringDF, select = c(numInternalDetails, manualInternalDetails))
S1_extScoreDF <- subset(S1_scoringDF, select = c(numExternalDetails, manualExternalDetails))
Rep_intScoreDF <- subset(Rep_scoringDF, select = c(numInternalDetails, manualInternalDetails))
Rep_extScoreDF <- subset(Rep_scoringDF, select = c(numExternalDetails, manualExternalDetails))

### Study 1
# Internal 
S1_intScoreDF$scaledManual <- (S1_intScoreDF$manualInternalDetails - mean(S1_intScoreDF$manualInternalDetails))/sd(S1_intScoreDF$manualInternalDetails)
S1_intScoreDF$scaledAuto <- (S1_intScoreDF$numInternalDetails - mean(S1_intScoreDF$numInternalDetails))/sd(S1_intScoreDF$numInternalDetails)
S1_intScoreDF_scaledDF <- data.frame(S1_intScoreDF$scaledManual, S1_intScoreDF$scaledAuto)
icc(S1_intScoreDF_scaledDF, model = "twoway", type = "agreement", unit = "single") # 0.628 

# External
S1_extScoreDF$scaledManual <- (S1_extScoreDF$manualExternalDetails - mean(S1_extScoreDF$manualExternalDetails))/sd(S1_extScoreDF$manualExternalDetails)
S1_extScoreDF$scaledAuto <- (S1_extScoreDF$numExternalDetails - mean(S1_extScoreDF$numExternalDetails))/sd(S1_extScoreDF$numExternalDetails)
S1_extScoreDF_scaledDF <- data.frame(S1_extScoreDF$scaledManual, S1_extScoreDF$scaledAuto)
icc(S1_extScoreDF_scaledDF, model = "twoway", type = "agreement", unit = "single") # 0.328

### Replication
# Internal 
Rep_intScoreDF$scaledManual <- (Rep_intScoreDF$manualInternalDetails - mean(Rep_intScoreDF$manualInternalDetails))/sd(Rep_intScoreDF$manualInternalDetails)
Rep_intScoreDF$scaledAuto <- (Rep_intScoreDF$numInternalDetails - mean(Rep_intScoreDF$numInternalDetails))/sd(Rep_intScoreDF$numInternalDetails)
Rep_intScoreDF_scaledDF <- data.frame(Rep_intScoreDF$scaledManual, Rep_intScoreDF$scaledAuto)
icc(Rep_intScoreDF_scaledDF, model = "twoway", type = "agreement", unit = "single") # 0.779

# External
Rep_extScoreDF$scaledManual <- (Rep_extScoreDF$manualExternalDetails - mean(Rep_extScoreDF$manualExternalDetails))/sd(Rep_extScoreDF$manualExternalDetails)
Rep_extScoreDF$scaledAuto <- (Rep_extScoreDF$numExternalDetails - mean(Rep_extScoreDF$numExternalDetails))/sd(Rep_extScoreDF$numExternalDetails)
Rep_extScoreDF_scaledDF <- data.frame(Rep_extScoreDF$scaledManual, Rep_extScoreDF$scaledAuto)
icc(Rep_extScoreDF_scaledDF, model = "twoway", type = "agreement", unit = "single") # 0.647

#### Create models: In this section, we predict the number of internal and external details from grandiosity, valence, and task
...
plot(allEffects(Rep_externalmodel), main = "Rep External Details", xlab = "Grandiosity", ylab = "# External Details")
...
#### Ensure that verbosity differences aren't masking these results
# To ensure that our results are not being impacted by differences in word count across these conditions, we add word count
# as a fixed effect. We later use a measure of internal and external density (number of details of a given category divided
# by the word count of the event) as our outcome variable of interest. Across both of these methods, we find no interaction
# of grandiosity and valence on objective detail, demonstrating that verbosity doesn't explain the lack of findings here.

##### WORD COUNT
# We first characterize the impact of task and valence on word count
aggregate(wordCount ~ taskName*valence, data = S1_fulldf, function(x) round(c(mean = mean(x), sd = sd(x)), 2))
aggregate(wordCount ~ taskName*valence, data = Rep_fulldf, function(x) round(c(mean = mean(x), sd = sd(x)), 2))

S1_wordCount_model <- lm(wordCount ~ taskName + valence, data = S1_fulldf)
summary(S1_wordCount_model) # significant impact of task on word count

Rep_wordCount_model <- lm(wordCount ~ taskName + valence, data = Rep_fulldf)
summary(Rep_wordCount_model) # significant impact of task on word count

# Create models regressing out word count as a fixed effect
S1_internalmodel_WC <- lmerTest::lmer(numInternalDetails ~ FFNI_Grandiose*valence*taskName + wordCount + (1|stim) + (1|subjectID), data = S1_fulldf)
plot(allEffects(S1_internalmodel_WC), main = "Study 1 Internal Details", xlab = "Grandiosity", ylab = "# Internal Details")
summary(S1_internalmodel_WC)

S1_externalmodel_WC <- lmerTest::lmer(numExternalDetails ~ FFNI_Grandiose*valence*taskName + wordCount + (1|stim) + (1|subjectID), data = S1_fulldf)
plot(allEffects(S1_externalmodel_WC), main = "Study 1 External Details", xlab = "Grandiosity", ylab = "# External Details")
summary(S1_externalmodel_WC)

Rep_internalmodel_WC <- lmerTest::lmer(numInternalDetails ~ FFNI_Grandiose*valence*taskName + wordCount + eventEmotionalTone + (1|stim) + (1|subjectID), data = Rep_fulldf)
plot(allEffects(Rep_internalmodel_WC), main = "Replication Internal Details", xlab = "Grandiosity", ylab = "# Internal Details")
summary(Rep_internalmodel_WC)

Rep_externalmodel_WC <- lmerTest::lmer(numExternalDetails ~ FFNI_Grandiose*valence*taskName + wordCount + eventEmotionalTone + (1|stim) + (1|subjectID), data = Rep_fulldf)
plot(allEffects(Rep_externalmodel_WC), main = "Replication External Details", xlab = "Grandiosity", ylab = "# External Details")
summary(Rep_externalmodel_WC)

sjPlot::tab_model(S1_internalmodel_WC, S1_externalmodel_WC)
sjPlot::tab_model(Rep_internalmodel_WC, Rep_externalmodel_WC)

##### DENSITY
#### Create models using detail density rather than raw detail number
S1_fulldf$internalDensity <- S1_fulldf$numInternalDetails/S1_fulldf$wordCount
S1_fulldf$externalDensity <- S1_fulldf$numExternalDetails/S1_fulldf$wordCount

Rep_fulldf$internalDensity <- Rep_fulldf$numInternalDetails/Rep_fulldf$wordCount
Rep_fulldf$externalDensity <- Rep_fulldf$numExternalDetails/Rep_fulldf$wordCount

## Study 1, Internal density
S1_internalmodel_density <- lmerTest::lmer(internalDensity ~ FFNI_Grandiose*valence*taskName + (1|stim) + (1|subjectID), data = S1_fulldf)
plot(allEffects(S1_internalmodel_density), main = "Study 1 Internal Density", xlab = "Grandiosity", ylab = "Internal Density")
summary(S1_internalmodel_density)

## Study 1, External
S1_externalmodel_density <- lmerTest::lmer(externalDensity ~ FFNI_Grandiose*valence*taskName + (1|stim) + (1|subjectID), data = S1_fulldf)
plot(allEffects(S1_externalmodel_density), main = "Study 1 External Density", xlab = "Grandiosity", ylab = "External Density")
summary(S1_externalmodel_density)

## Replication, Internal
Rep_internalmodel_density <- lmerTest::lmer(internalDensity ~ FFNI_Grandiose*valence*taskName + eventEmotionalTone + (1|stim) + (1|subjectID), data = Rep_fulldf)
plot(allEffects(Rep_internalmodel_density), main = "Rep Internal Density", xlab = "Grandiosity", ylab = "Internal Density")
summary(Rep_internalmodel_density)

## Replication, External
Rep_externalmodel_density <- lmerTest::lmer(externalDensity ~ FFNI_Grandiose*valence*taskName + eventEmotionalTone + (1|stim) + (1|subjectID), data = Rep_fulldf)
plot(allEffects(Rep_externalmodel_density), main = "Rep External Density", xlab = "Grandiosity", ylab = "External Density")
summary(Rep_externalmodel_density)
